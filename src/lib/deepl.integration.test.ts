...
// Skip all tests in jsdom environment since axios doesn't work there for external APIs
const isJSDOMEnvironment = typeof window !== 'undefined' && window.navigator?.userAgent?.includes('jsdom');

// Integration Tests - These run against the actual DeepL API
// Set DEEPL_API_KEY environment variable to run these tests
// Note: These tests require a Node.js environment, not jsdom
describe("DeepL API Integration Tests (Real API)", () => {
    const apiKey = process.env.DEEPL_API_KEY;
    const isProTier = process.env.DEEPL_PRO === "true";

    // Skip these tests if no API key is provided OR if running in jsdom
    const testIf = (condition: boolean) => (condition && !isJSDOMEnvironment) ? it : it.skip;

    if (isJSDOMEnvironment) {
        console.log(`
⚠️  DeepL Integration Tests skipped in jsdom environment.
   To run these tests, use a Node.js test environment:
   ...
   npm test deepl.integration.test.ts -- --environment=node
    ...
    describe("Real API Translation Tests", () => {
        ...
        testIf(!!apiKey)("should translate Japanese kanji to German", async () => {
            ...
            expect(typeof result).toBe("string");
            expect(result.length).toBeGreaterThan(0);
            // Should translate 水 (water) to "Wasser" in German
            expect(result.toLowerCase()).toMatch(/wasser/);
        });
        ...
        testIf(!!apiKey)("should handle multiple kanji for radicals context", async () => {
            ...
            const tests = [
                { input: "人", expectedMatch: /mensch|person/ },
                { input: "大", expectedMatch: /groß|große/ },
                { input: "小", expectedMatch: /klein|kleine/ }
            ];

            for (const test of tests) {
                const result = await translateText(apiKey!, test.input, "DE", isProTier);

                expect(result).toBeDefined();
                expect(typeof result).toBe("string");
                expect(result.toLowerCase()).toMatch(test.expectedMatch);
    ...
    describe("Error Handling with Real API", () => {
        ...
        // Skip this test in jsdom as it causes CORS errors
        testIf(!!apiKey && !isJSDOMEnvironment)("should fail with invalid API key", async () => {
            const { translateText } = await import("./deepl");
...
🔧 To run DeepL integration tests:
   ...
   3. Run with Node.js environment: npm test deepl.integration.test.ts -- --environment=node
   
   Alternative - Set environment variables directly:
   
   PowerShell Example:
   $env:DEEPL_API_KEY="your_api_key_here"; npm test deepl.integration.test.ts -- --environment=node
   
   CMD Example:
   set DEEPL_API_KEY=your_api_key_here && npm test deepl.integration.test.ts -- --environment=node
   
   Bash Example:
   DEEPL_API_KEY="your_api_key_here" npm test deepl.integration.test.ts -- --environment=node
        `);
    }
});
